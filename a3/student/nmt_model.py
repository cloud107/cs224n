...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(in_channels = embed_size, out_channels = embed_size, padding = 'same', kernel_size = 2)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        P = F.log_sotmax(self.target_vocab_projection(combined_outputs), dim=-1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        init_decoder_hidden = last_hidden.permute(1,0,2).reshape(last_hidden.shape[1],-1)
        init_decoder_hidden = self.h_projection(init_decoder_hidden)
        init_decoder_cell = last_cell.permute(1,0,2).reshape(last_cell.shape[1],-1)
