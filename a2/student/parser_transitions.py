...
def minibatch_parse(sentences, model, batch_size):
    ...
    sentences_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = sentences_parses[:]
    ...
    while unfinished_parses:
        ...
        for pp, t in zip(batch, transitions):
            if t == "S":
                pp.parse_step(t)
            elif t == "LA" and len(pp.stack) >= 2:
                pp.parse_step(t)
            elif t == "RA" and len(pp.stack) >= 2:
                pp.parse_step(t)
        # 完成条件的 parse 不再进入下一轮;对象本身不用删,只是列表不要它了
        
        unfinished_parses = [pp for pp in unfinished_parses
                            if not (len(pp.stack) == 1 and not pp.buffer)]

    dependencies = [pp.dependencies for pp in sentences_parses]
