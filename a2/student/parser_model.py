...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        x = torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias  # (batch_size, hidden_size)
        x = F.relu(x)  # (batch_size, hidden_size)
        x = self.dropout(x)  # (batch_size, hidden_size)
        logits = torch.matmul(x, self.hidden_to_logits_weight) + self.hidden_to_logits_bias  # (batch_size, n_classes)
